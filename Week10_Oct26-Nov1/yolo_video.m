...
while hasFrame(v)
        ...
        image = single(imresize(readFrame(v),[448 448]))/255;
        %image = single(imresize(imread('person_horse.png'),[448 448]))/255;
        %figure(1);
        %imshow(image);
...
%             for i = 1:length(boxes)
%                 if classLabels(boxes(i).classIndex) == 'person'
%                 image = insertObjectAnnotation(image,'rectangle',boxes(i).coords,convertStringsToChars(classLabels(boxes(i).classIndex)));
%                 end
%             end
%             figure(1)
%             imshow(image);
...
%                 image = insertObjectAnnotation(image,'rectangle',nonIntersectBoxes(i).coords,convertStringsToChars(classLabels(nonIntersectBoxes(i).classIndex)));
%                 end
                 if nonIntersectBoxes(i).nonMax == 1
                      %if classLabels(nonIntersectBoxes(i).classIndex) == 'person' 
