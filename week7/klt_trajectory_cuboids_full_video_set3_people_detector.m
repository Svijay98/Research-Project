...
nei_len = 31;
video_dataset_path = 'KTH_dataset'; % Path for the dataset
...
for sf = 1:numel(sub_folders)
    ...
    for f = 1:numel(all_files_in_subfolder)
        ...
        while (total_frames - frame_no) >= 15
                ...
                [bboxes,scores] =  peopleDetector(prev_frame);
                ...
                prev_features = detectHarrisFeatures(prev_frame,'ROI',bboxes(1,:));
                ...
                parfor i = 1:num_of_xy
                    ...
                    %temp = cat(2,temp,sf);
                    dlmwrite('descriptors_t+mbh+pd_all_classes.csv',temp,'-append','delimiter',',','precision',4); % Storing the descriptors in a csv file
                end
        ...
        dlmwrite('video_feature_count_pd.csv',video_feature_count,'-append','delimiter',',','precision',4);
        toc
    end
