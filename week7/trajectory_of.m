clc;
nei_len = 31;
video_dataset_path = 'C:\Users\Sanjana\Desktop\Research Project\training_dataset'; % Path for the dataset
    ...
    for f = 1:numel(all_files_in_subfolder)
        ...
        while cnt < total_cuboids
                ...
                while frame_len > 0
                    ...
                    flow = estimateFlow(opticalFlow,curr_frame);
                    for row=1:size(flow.Orientation,1)
                        for column=1:size(flow.Orientation,2)
                            w=[flow.Vx(row,column),flow.Vy(row,column)];
                            temp = medfilt2(w);
                            curr_points = prev_features.Location + temp;
                        end
                    end
