    ...
    for f = 1:numel(all_files_in_subfolder)
        ...
        while (total_frames - frame_no) >= 15
                ...
                parfor i = 1:num_of_xy
                    ...
                    %temp = cat(2,temp,sf);
                    dlmwrite('descriptors_harris_t+hog+hof_all_classes.csv',temp,'-append','delimiter',',','precision',4); % Storing the descriptors in a csv file
                end
        end
        video_feature_count =[];
        ...
        dlmwrite('video_feature_count.csv',video_feature_count,'-append','delimiter',',','precision',4);
        toc
    end
